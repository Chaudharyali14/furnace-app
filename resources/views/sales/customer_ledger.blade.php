...
@section('title', __('messages.customer_ledger'))

@section('content')
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>{{ __('messages.customer_ledger') }}</h1>
        <a href="{{ route('sales.index') }}" class="btn btn-secondary">{{ __('messages.back_to_sales_overview') }}</a>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-header">{{ __('messages.select_customer') }}</div>
        <div class="card-body">
    ...
    @if ($selectedCustomer && $selectedCustomer->opening_balance == 0)
        <div class="card mt-4">
            <div class="card-header">
                <h5>{{ __('messages.set_opening_balance') }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('customer.ledger.opening_balance') }}">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="opening_balance_type" id="opening_balance_type" class="form-select" required>
                                <option value="debit">{{ __('messages.debit_customer_owes_you') }}</option>
                                <option value="credit">{{ __('messages.credit_you_owe_customer') }}</option>
                            </select>
                        </div>
                    </div>
    ...
    @if ($selectedCustomer)
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4>{{ __('messages.ledger_for') }}: {{ $selectedCustomer->name }}</h4>
                <div>
                    <button id="printBtn" class="btn btn-secondary me-2"><i class="fas fa-print"></i> {{ __('messages.print') }}</button>
                    <button id="excelBtn" class="btn btn-success"><i class="fas fa-file-excel"></i> {{ __('messages.excel') }}</button>
                </div>
            </div>
            <div class="card-body">
                ...
                <form method="GET" action="{{ route('customer.ledger') }}" class="mb-4">
                    <input type="hidden" name="customer_id" value="{{ $selectedCustomer->id }}">
                    <div class="row g-2">
                        <div class="col-md-3">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="customer-ledger-table">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">{{ __('messages.no_transactions_found_for_this_customer') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('customer.ledger.payment') }}">
                    @csrf
                    <input type="hidden" name="customer_id" value="{{ $selectedCustomer->id }}">
                    <div class="row">
                        <div class="col-md-6 mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('printBtn').addEventListener('click', function () {
            const table = document.getElementById('customer-ledger-table').outerHTML;
        ...
        document.getElementById('excelBtn').addEventListener('click', function () {
            const table = document.getElementById('customer-ledger-table');
            const wb = XLSX.utils.table_to_book(table, {sheet: "Customer Ledger"});
